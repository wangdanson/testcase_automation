...
* **雙標籤強制規範**：每個測試功能必須使用兩類標籤，格式為 `【分類標籤】【性質標籤】`。
    * **分類標籤 (維度)**：
        * `【權限】`：帳號屏蔽、角色特權、隔離驗證。
        * `【功能】`：業務操作、欄位必填、邏輯判斷。
        * `【狀態】`：狀態轉移 (Oncue/Paused)、受眾生命週期、預算釋放。
        * `【數據】`：報表輸出、指標語意、Excel 格式。
        * `【整合】`：跨系統同步 (Studio/ERP)、Sequence 產出、API 結構。
        * `【量測】`：追蹤碼設定、第三方量測 (IAS/Moat/DV)。
        * `【UX】`：介面佈局、L10n、操作回饋 (Tooltip/彈窗)。
        * `【排程】`：00:00/05:00 定時檢查、批次自動化邏輯。
    * **性質標籤 (屬性)**：`【正向】`、`【反向】`、`【回歸】` (隔離驗證)、`【相容】` (歷史數據)、`【格式】`、`【邊界】`。
* **範例**：`【權限】【回歸】驗證一般帳號維持舊版編輯介面`。
