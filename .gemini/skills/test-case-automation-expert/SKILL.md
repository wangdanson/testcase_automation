...
* **測項欄位精準度規範**：
    * `功能/頁面` 必須填 QA 實際進入或開始操作的頁面/模組；不要為了技術驗證另造不存在或 QA 找不到的功能入口。
    * `測試功能` 必須是短標題，描述要測的功能點或限制條件，不把完整情境、預期結果或後端欄位名稱包成標題。
    * `測試情境` 必須說明 QA 要驗證的真實場景，且不可把規格沒有要求的 UI 呈現推論成可見行為。
    * `操作步驟` 必須包含入口、頁面、欄位或操作方式；若驗證點在 API、報表、投放輸出、Sequence、ODM 或其他系統，步驟中必須明確寫出查核位置。
    * `期望結果` 必須可觀察、可比對、可判定通過或失敗；需清楚區分 UI 顯示、資料保存、API 回應、報表內容、Sequence 輸出與跨系統狀態。
    * `備註` 必須標明來源或驗證方式；若測項來自後端資料來源或輸出規則，要寫清楚「不是 UI 顯示測試」。
    * 禁止過度延伸：例如規格只寫 `customers.mobile_url`、`mobile_url` 或「媒體 URL 使用手機版」時，測項應描述「資料來源/儲存資料/投放輸出/Sequence 使用 mobile URL」，不得寫成「使用者可辨識手機版 URL」，除非規格明確要求 UI 顯示。
...
* **可執行性審計**：逐列檢查 QA 是否能從 `功能/頁面` 與 `操作步驟` 找到入口並完成操作；若步驟只寫「檢查資料」但沒有說明查 UI、API、報表、投放輸出或 Sequence，必須補明確。
* **驗證層級審計**：逐列確認測項屬於 UI、API、報表、Sequence、ODM 或跨系統驗證中的哪一種；不得把後端資料來源或 API 欄位錯寫成 UI 可見內容。
...
- [ ] 每筆測項是否讓 QA 明確知道入口、操作方式、驗證位置與通過標準？
- [ ] 是否已區分 UI 顯示、API 回應、報表內容、Sequence 輸出與跨系統狀態，避免把技術資料來源過度延伸成 UI 行為？
- [ ] 規格未明確要求 UI 顯示時，是否避免寫成「使用者可看到/辨識」類測項？
